    ...
    while (1)
    {
        ...
        if (header.flag == static_cast<unsigned char>(0) && checkSum((u_short*)Buffer, length - sizeof(header)))
        {
            ...
            if (checkSum((u_short*)Buffer, length - sizeof(header)) == 0 && seq != int(header.SEQ))
            {
                ...
                cout << " 重发上一个回复包 - ACK:" << (int)header.SEQ << " SEQ:" << (int)header.SEQ << endl;
                continue; // 丢弃该数据包
            }
            seq = int(header.SEQ);
            if (seq > 255)
            ...
            cout << " 收到了 " << length - sizeof(header) << " 字节 - Flag:" << int(header.flag) << " SEQ:" << int(header.SEQ) << " SUM:" << int(header.sum) << endl;
            ...
            // 返回客户端ACK，代表成功收到了
            sendto(sockServ, Buffer, sizeof(header), 0, (sockaddr*)&ClientAddr, ClientAddrLen);
            cout << " 回复客户端 - flag:" << (int)header.flag << " SEQ:" << (int)header.SEQ << " SUM:" << int(header.sum) << endl;
            seq++;
            if (seq > 255)
            {
                seq = seq - 256;
